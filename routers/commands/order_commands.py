...
from keyboards.order_keyboard import (get_open_order_keyboard, get_choose_menu_keyboard, get_payment_keyboard,
                                      get_close_order_keyboard, get_price_option_keyboard)
from fsm.shift_fsm import  OrderStates
...
@router.callback_query(F.data.in_({"position_menu_medium", "position_menu_light", "position_menu_fruit"}))
async def select_hookah(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Спрашиваем, оставить ли цену или изменить
...
@router.callback_query(F.data == "leave_price")
...
async def enter_custom_price(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer("Пожалуйста, введите корректную положительную числовую цену.")

@router.message(OrderStates.enter_comment)
async def enter_comment(message: types.Message, state: FSMContext):
    ...
    # Добавление комментария в состояние
